...
import           Control.Exception                         (Handler(..), IOException, SomeException, catches)
import           Control.Monad.IO.Class
import           Data.Typeable                             (typeOf)
import           Prelude                                   hiding (catch)

main :: IO ()
main = guarded `catches`
       [
         Handler (\ex -> putStrLn ("wandexception:" ++ show (ex :: ImageWandException))),
         Handler (\ex -> putStrLn ("ioexception:" ++ show (ex :: IOException))),
         Handler (\ex -> putStrLn ("someexception:" ++ show (ex :: SomeException)))
       ]
